...
const commands = [
    {
        label: "中-英-德词汇互译",
        content: `你是一位很出色的语言翻译大师。
请自动检测输入文本中单引号（'）包裹的内容所使用的语言（仅限中文、英文或德语三种语言）。
检测完成后，将该内容翻译成另外两种语言，并分别为每种语言提供一个例句，例句后附上中文翻译。
...
2.内容与风格： 故事情节需完整、简单易懂，并自然地融入与关键词相关的词汇及表达。请务必使用简洁的语言和常用词汇。语法要求：避免使用过去时，优先使用使用现在时和完成时。
...
`,
    ...
    },
    {
        ...
        避免使用复杂的语法结构和生僻词汇，但可以使用完成时（因为日常生活中德国人习惯用完成时）。
        对话长度大约为300字。 
    ...
    {
        label: "词根词源相似词",
        content: `请对单引号内的德语词汇提供以下解析（全部使用中文）：

1. **词汇释义**  
   - 用简明语言解释核心含义，若有多义项请分点说明（标注常见程度 ★★☆）  
   - 提供1-2个A1-B1难度的实用例句（标注场景如【日常/书面】）

2. **词源拆解**  
   - 分解词根/词缀，说明历史来源（如拉丁语、古高地德语等）  
   - 例："*Beispiel*" = *bei-*（靠近） + *Spiel*（游戏）→ "贴近游戏的示范"

3. **形近词对比**  
   - 列出3-5个常见易混词（标注词性差异）  
   - 例："*verstehen*（动）↔ *Verstand*（名，理智）"

4. **扩展联想**  
   - 推荐2-3个同词族的高频词（如动词名词化形式）`,
        placeholder: "请输入要解释的词汇...",
        Temperature: '0.3',// 翻译任务需要更高的确定性
        topP: '0.7',
        topK: '20'
    ...
    {
        label: "德语精读",
        content: `"单引号中的是德语内容，用【中文】对它进行详细讲解：
        1. 词汇方面：
            解释每个单词的词性、含义，以及常见的搭配用法。
            如果单词有多种含义，请根据上下文指出其在本句中的具体含义。
            如果句子中包含可分动词，请指出其分离的前缀和动词原形，并解释其含义。
            提供难度在 A1-B1 级别的德语例句，展示该词汇在不同语境下的用法，并附上中文翻译。
        2. 固定搭配方面：
            解释句子中使用的固定搭配或习语，说明其含义和用法。
            提供难度在 A1-B1 级别的德语例句，展示该搭配在不同语境下的用法，并附上中文翻译。
        3. 语法方面：
            分析句子的结构（主语、谓语、宾语、状语等）。
            解释句子的时态、语态（主动/被动）、语气（陈述/疑问/命令）等。
            指出句子中使用的特殊句型或结构，例如：
            从句 (Nebensatz) 的类型和引导词
            情态动词 (Modalverben) 的用法
            可分动词 (Trennbare Verben) 的结构
            反身动词 (Reflexive Verben) 的用法
            被动语态 (Passiv) 的构成
            第二虚拟式 (Konjunktiv II) 的用法
            分析句子中是否存在固定搭配 (feste Redewendungen) 或习语 (Idiome)，并解释其含义。
        4. 整体解释：
            用简明易懂的【中文】总结句子的整体意思。
            如果句子有特殊的语用含义（例如讽刺、幽默等），请指出。
        5. 目标：
            使讲解既能帮助德语学习者理解句子的每个组成部分，又能把握句子的整体含义和语用功能。"
        ...
        示例：
            ...
            Ich: 人称代词，我 (第一人称单数主格)。 例句：Ich bin müde. (我累了。) - A1
            habe: 动词 haben (有) 的第一人称单数现在时形式，这里作为助动词，构成现在完成时。 例句：Ich habe ein Auto. (我有一辆车。) - A1
            gestern: 副词，昨天。 例句：Gestern war Sonntag. (昨天是星期天。) - A1
            ein: 不定冠词，阳性/中性第四格，这里修饰 Buch。 例句：Ich habe ein Haus. (我有一栋房子。) - A1
            Buch: 名词，das Buch (中性)，书。 例句：Das Buch ist interessant. (这本书很有趣。) - A1
            gelesen: 动词 lesen (读) 的过去分词形式，与 habe 构成现在完成时。 例句：Ich lese ein Buch. (我在读书。) - A1
            ------------------------
            固定搭配：

            1. "ein Buch lesen" (读一本书) 是一个常见的搭配，表示阅读一本书籍。例句：Ich lese jeden Abend ein Buch. (我每天晚上读一本书。) - A1
           ------------------------
            语法精讲：
            句子结构：主语 (Ich) + 谓语 (habe gelesen) + 宾语 (ein Buch) + 时间状语 (gestern)。
            时态：现在完成时 (Perfekt)，由助动词 haben + 过去分词 gelesen 构成，表示过去的动作。
            语态：主动语态 (Aktiv)。
            句子中没有固定搭配或特殊句型。
            ------------------------
            整体解释：
            这句话的意思是“我昨天读了一本书”。 这是一个简单的陈述句，描述了说话者昨天完成的读书行为。
        `,
        placeholder: "请输入要解释的词汇...",
        Temperature: '0.3',// 翻译任务需要更高的确定性
        topP: '0.7',
    ...
    {
        label: "德语词句简析",
        content: "单引号中的是一些德语句子或/和单词，请在每句或单词后面添加中文翻译和解释。标记重点词汇，如果该词是变形而来的，那么请出给原始形式以及含义。",
        placeholder: "输入德语句子或单词",
        Temperature: '0.8',// 翻译任务需要更高的确定性
        topP: '0.8',
        topK: '30'
    },
    ...
    {
        label: "逐词垂直翻译",
        content: `单引号中是德语，请逐词翻译成中文。
        格式要求：
        1.第一行是德语原文。
        2.第二行是对应每个德语单词的中文翻译。
        3.每个德语单词和其下方的中文翻译垂直对齐。
        4.der,das,die等以冠词形式存的时候，不需要翻译，如果是以代词的形式出现，翻译出代指的是谁。
        `,
        placeholder: "请输入中文句子",
        Temperature: '0.7',
        topP: '0.9',
